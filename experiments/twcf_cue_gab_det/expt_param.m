...
p.stim.gab.orientation = [-45 45]; % possible orientations
p.stim.gab.phase = 0; 
...
p.stim.gab.noiseContrast = 0.5; % 0.5 0.2; 0.08 (Rahnev) 

% priors for line lengths corresponding to
% - extreme low and high values (indices 1 and 7)
...
p.stim.gab.contrastPrior_list = [0.0441    0.0817    0.0981    0.1123    0.1269    0.1457    0.2699]; 
...
if setup.isMainExpt
    ...
    p.stim.gab.contrast_list = expt_getThresholds(setup.threshDir, [], p);
...
elseif setup.isPiloting 
    ...
    p.stim.gab.contrast_list = logspace(log10(p.stim.gab.gratingContrast_min), log10(p.stim.gab.gratingContrast_max) ,7); % logspace(log10(1-p.stim.gab.noiseContrast),log10(0.05),7); 
...
p.quest.tGuess     = log10(0.12); 
p.quest.tGuessSD   = 3; % recommended 

% beta = psychometric function slope
p.quest.beta = 3.649; % 3.3438
