function [data, timing, gaze, exitNow] = expt_runTrial(w, p, b, data, timing, gaze, i_index, i_trial, setup)

%% Experiment 1.4 cue gabor discrimination 
%% trial stages
...
if p.setup.isThresholding
    ...
    %%% get current QUEST estimate of w_tilt and ensure it's within [0,45]
    % w_tilt = 0 -> mininum tilt 
    % w_tilt = 1 -> maximum tilt (±45, same as in detection task) 
    % w_tilt is the weight to multiply by 45° to determine the threshold tilt in deg

    w_tilt = 10.^QuestMean( data.q_tilt( data.qID(i_trial) ) );

    if     w_tilt < 0, w_tilt = 0.001; % set lower min 
    elseif w_tilt > 1, w_tilt = 1; end % set upper bound 
    
    data.q_tilt( data.qID(i_trial)).intensity(i_trial) = log10(w_tilt); % try 
    data.w_tilt(i_trial) = w_tilt;

    % initial FG rects
    ovalVert  = [0, 0, p.stim.periph.circleWidth_inPix, p.stim.periph.circleWidth_inPix]; % ovalMinorAxis_inPix, ovalMajorAxis_inPix]; % CHANGE to CIRCLE 
    ovalHoriz = [0, 0, p.stim.periph.circleWidth_inPix, p.stim.periph.circleWidth_inPix]; % ovalMajorAxis_inPix, ovalMinorAxis_inPix];
    
    % save contrast to data structure 
    ...
    % save contrast to s structure 
    s.contrast_all   = s.gab.contrast_list( data.contrastID_all(:, i_trial) );
    s.contrastID_all = data.contrastID_all(:, i_trial);

else % main, practice, validation, piloting
    data.w_tilt(i_trial) = p.stim.periph.w_tilt; % should load individually calibrated tilt 
...
s.w_tilt = data.w_tilt(i_trial); % save for expt_makeStimGabPeriph? 
data.tilt_inDeg(i_trial) = max(p.stim.gab.orientation) * data.w_tilt(i_trial); 
...
if i_trial == 1
    ...
    if data.passCurrent > data.pass(i_trial-1)
        ...
        % keypress
    elseif setup.buttons==2
        flipTime = timing.keypress2(i_trial-1) + p.timing.true_ITIDur_inSecs;
    else
...
% response 1: orientation and reference comparison 
...
if p.setup.isThresholding && (data.correctDis(i_trial) >= 0)
    data.q_tilt( data.qID(i_trial) ) = QuestUpdate( data.q_tilt( data.qID(i_trial) ), log10(data.w_tilt(i_trial)), data.correctDis(i_trial) );
end


%% collect and process keyboard input 
if setup.buttons==2
    % response 2: orientation visibility (did you see the orientation? yes/no)

    % listen for keyboard input
    try
        [respKey2, data.RT2(i_trial), timing.keypress2(i_trial)] = ...
            recordValidKeys(timing.keypress(i_trial), p.timing.postcue2Dur_inSecs, p.kb.kbNum, p.kb.allowedKeys2);

        % handle keypress timestamps when no keypress was successfully entered.
        % this is necessary b/c the onset of the following trial is timed
        % relative to timing.keypress(i_trial)
        if timing.keypress2(i_trial) == 0
            data.RT2(i_trial) = NaN;
            timing.keypress2(i_trial) = GetSecs;
        end

        % check if respKey is valid input to a switch statement
        if ~isscalar(respKey2) && ~ischar(respKey2)
            respKey2 = 'error';
            data.RT2(i_trial) = NaN;
            timing.keypress2(i_trial) = GetSecs;
        end
    ...
    switch respKey2
        ...
        % valid responses
        case p.kb.allowedKeys2(1),   data.ratingOri(i_trial) = 0;  data.resp2(i_trial) = 0; % did not see orientation 
        case p.kb.allowedKeys2(2),   data.ratingOri(i_trial) = 1;  data.resp2(i_trial) = 1; % saw orientation 

        % invalid responses / errors / etc
        case 'noanswer',            data.resp2(i_trial) = -1;
        case 'invalid',             data.resp2(i_trial) = -2;
        case 'cell',                data.resp2(i_trial) = -3;
        case p.kb.exitKey,          data.resp2(i_trial) = -4;   exitNow = 1;
        otherwise,                  data.resp2(i_trial) = -5;
    end

    data.validKeypress2(i_trial) = data.resp2(i_trial) >= 0;

    if exitNow, return; end


    %% post-response processing
    % response 2: orientation and reference comparison

    % flash crosshair color change after response is entered
    if data.validKeypress2(i_trial)
        Screen('FillRect', w, p.stim.fixationColor{3}, p.rects.fixation); % draw fixation cross to white 
        for i_loc = 1:4
            Screen('DrawTexture', w, tex_cue_inactive, [], p.rects.cueRect(:, i_loc), rad2deg(p.rects.angleList_inRadians(i_loc))+90);
        end

        if helpText
            text = 'post-response processing2';
            DrawFormattedText(w, text, p.text.sx, p.text.sy, 0, p.text.wrapat);
        end
...
if setup.buttons==2
    disp(['orientation visibility rating = ' num2str(data.resp2(i_trial))]);
end
if p.setup.isThresholding && (data.correctDis(i_trial) >= 0)
    disp(['estimated threshold = ' num2str(w_tilt)]);
end
disp(' ');
...
if b.practiceStage == 2
    ...
    % show text feedback
    stimText = upper( {'no grating', 'counterclockwise from vertical', 'clockwise from vertical'} );
    respText = upper( {'counterclockwise from vertical - more visible than reference',...
        'counterclockwise from vertical - less visible than reference',... 
        'clockwise from vertical - less visible than reference',...
        'clockwise from vertical - more visible than reference'} );
    if setup.buttons==2
        respText2 = upper( {'guessed direction of tilt'...
            'saw direction of tilt'} ); 
    end

    if setup.buttons==2
        fbText   = ['At the cued location, the stimulus was: ' stimText{ data.stimID_postcue(i_trial)+1 } '\n\n' ...
            'Your 1st response was: ' respText{ data.resp(i_trial) } '\n\n' ...
            'Your 2nd response was: ' respText2{ data.ratingOri(i_trial)+1 } '\n\n' ... 
...
if setup.buttons==2
    data.goodTrial(i_trial)   = data.validKeypress(i_trial) & data.validKeypress2(i_trial); 
else
