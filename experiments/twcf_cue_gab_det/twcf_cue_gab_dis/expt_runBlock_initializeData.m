function [data, timing, gaze, progress] = expt_runBlock_initializeData(p, b, setup)
...
data.stimID_all               = nan(4, nBuffer); % 0 -->none, 1--> -45, 2--> 45
data.contrastID_all           = nan(4, nBuffer);
data.contrast_all             = nan(4, nBuffer); 
...
data.contrastID_postcue       = nan(1, nBuffer); 
data.contrast_postcue         = nan(1, nBuffer); 

data.w_tilt                   = nan(1, nBuffer); 
data.tilt_inDeg               = nan(1, nBuffer); 

% response variables
data.respDis                  = nan(1, nBuffer);
% data.respDet                  = nan(1, nBuffer);
data.ratingDis                = nan(1, nBuffer);
data.resp                     = nan(1, nBuffer);
data.RT                       = nan(1, nBuffer);
data.correctDis               = nan(1, nBuffer);
% data.correctDet               = nan(1, nBuffer);
data.ratingOri                = nan(1, nBuffer);

% response variables (button press 2) 
if setup.buttons==2
    data.resp2                = nan(1, nBuffer);
    data.RT2                  = nan(1, nBuffer);
    data.ratingOri            = nan(1, nBuffer);
    data.goodTrial2           = nan(2, nBuffer);
end
...
if p.setup.isThresholding
    ...
    for i_track = 1:3
        data.q_tilt(i_track) = QuestCreate(p.quest.tGuess, p.quest.tGuessSD, p.quest.pThreshold, p.quest.beta, ...
                                      p.quest.delta, p.quest.gamma, p.quest.grain, p.quest.range);
    end
end
...
timing.keypress2           = nan(1, nBuffer); 
