function contrast_list = expt_getThresholds(threshDir, makePlot, p)
% contrast_list = expt_getThresholds(threshDir, makePlot, p)
%
% Returns threshold values for contrast based on a subject's most recent
% thresholding block. The output contrast_list contains contrast
% thresholds for the following 7 points:
%
% - Lower extreme: contrast midway between minimum contrast (0.0001???) and 
% contrast yielding nearly-chance performance (.51) on a log10 scale. Define 
% displacement D = (log10(len(pcorr=0.51)) - log10(len(3 pixels)))/2. Then 
% log10(len(lower extreme)) = log10(len(pcorr=0.51)) - D
% - p(correct gabor orientation discrimination) = 0.6
% - p(correct gabor orientation discrimination) = 0.675
% - p(correct gabor orientation discrimination) = 0.75
% - p(correct gabor orientation discrimination) = 0.825
% - p(correct gabor orientation discrimination) = 0.9
...
% thresholding hasn't been performed yet), contrast_list is
% returned as NaN.
%
% INPUTS
...
x_min = log10(0.0078); % log10(p.screen.contrastRes); 
x_max = log10(1 - p.stim.gab.noiseContrast); % log10(1 - p.stim.gab.noiseContrast); 
...
% determine the two extreme endpoint line lenghts as follows (textures): 
%
...
% compute contrast corresponding to 51% correct
...
D = (x_max - x_target(end))/2;
x_target_extr_D(2) = x_target(end) + D;

% define lower extreme
D2 = x_target_extr_D(2) - x_target(end); % D2 is equal to D in this scenario 
x_target_extr_D(1) = x_target(1) - D2;
% check if lower extreme too low:
if x_target_extr_D(1) < x_min
    x_target_extr_D(1) = x_min; 
end
...
if makePlot
    ...
    xlim([min(x), max(x)])
    xlabel('log_{10}(contrast) at cued loc (deg)')
    ...
    plot(x_min*[1,1], ylim, 'b-') % minimum contrast
    plot(x_max*[1,1], ylim, 'b-') % maximum contrast
