function [gab_stim, numGenerated] = expt_makeStimGabPeriph(w, p, s)
...
%         end
        switch p.stim.gab.noiseType
            ...
            case 'gaussian' % normally distributed noise centered around mean and sd - outdated
                noiseSD = 0.3;
...
%     end
    ...
    %% Draw gabor  
    % Get contrasts
    gratingContrast = s.contrast_all(i);
    noiseContrast = p.stim.gab.noiseContrast;
...
%     end
    ...
    if s.stimID_all(i) == 0 % Target absent
        % Center noise on 0.5 and scale to specified contrast 
