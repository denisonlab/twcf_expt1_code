...
stimIDs{1} = [2; 1; 2; 2]; % change to all present? 
stimIDs{2} = [1; 2; 2; 1];
stimIDs{3} = [1; 1; 2; 1];
stimIDs{4} = [2; 2; 1; 2]; 
...
textOval = {'1 key\n\nmore visible than reference\n\ncounterclockwise\nfrom vertical',...
    '2 key\n\nless visible than reference\n\ncounterclockwise\nfrom vertical',...
    '9 key\n\nless visible than reference\n\nclockwise\nfrom vertical',...
    '0 key\n\nmore visible than reference\n\nclockwise\nfrom vertical'};
...
textOval2 = {'3 key\n\nguessed direction of the grating tilt',...
    '8 key\n\nsaw direction of the grating tilt'};
for i = 1:numel(textOval2)
    [textTextureOval2(i), textRectOval2{i}] = makeTextTexture(w, textOval2{i}, padding, resetBGcolor, fontScaleMult);
end
...
ind = [2 3 7 8];
filenames_resp1 = {'grating1','grating2','grating9','grating0'}; 
for i = 1:4 % first response cartoon 
    i_filename       = sprintf('supporting_files/%s.png', filenames_resp1{i}); 
    [oval, ~, alpha] = imread(i_filename);
    oval(:, :, 4)    = alpha;
    ...
    offset = RectHeight(ovalRect{1})/2 + .6*RectHeight(ovalRect{1});
...
ind = [4 6]; 
filenames_resp2 = {'grating3','grating8'}; 
for i = 1:2 % second response cartoon
    i_filename       = sprintf('supporting_files/%s.png', filenames_resp2{i}); 
    [oval2, ~, alpha] = imread(i_filename);
    oval2(:, :, 4)    = alpha;
    ovalTex2(i)       = Screen('MakeTexture', w, oval2);

    ovalRectScale     = p.rects.periphFG{ori(i)}(:,1)'; 
    heightOffset      = (ovalRectScale(4)-ovalRectScale(2))/2; 
    ovalRectScale(2)  = ovalRectScale(2)-heightOffset; 
    ovalRectScale(4)  = ovalRectScale(4)+heightOffset; 

    ovalRect2{i}       = CenterRectOnPoint( ceil(ovalRectScale*.85), centers(ind(i)), p.rects.midH);

    offset = RectHeight(ovalRect2{1})/2 + .6*RectHeight(ovalRect2{1});
    textRectOval2{i} = CenterRectOnPoint( textRectOval2{i}, centers(ind(i)), p.rects.midH);
    textRectOval2{i} = OffsetRect( textRectOval2{i}, 0, offset);
end
...
page{i}     = ['INSTRUCTIONS\n\n* Overview\n\n' ...
               ...
               'Specifically, you will be pointed or "cued" to a particular location on the screen and will have to report:\n\n' ...
                '- What was the tilt of the grating (counterclockwise or clockwise from vertical)\n' ... 
                '- Was the grating more or less visible (i.e. did it "pop-out" more or less) than a reference?\n' ...
                '- Did you guess or see the direction of the grating tilt?\n\n' ...
...
i = i + 1;
page{i}     = ['* Gratings\n\nThe noise patches will contain a grating.\n\n' ...
               'A grating is a set of oriented lines.\n\n' ...
               'In the experiment, a grating can be oriented either counterclockwise or clockwise from vertical.\n\n'];
...
page{i}     = ['* Quadrants\n\nIn the main experiment, the screen will be split into 4 sections, called quadrants.\n\n' ...
               'A different noisy grating patch will be shown in each quadrant.\n\n' ...
               'The gratings may be independently oriented counterclockwise or clockwise from vertical.\n\n'];
...
page{i}     = ['* Response cue (shown after the 4 quadrants)\n\nAfter the noise patches in the 4 quadrants are shown, one of ' ...
               ...
               'For the response-cued patch, you will enter using 2 keypresses, 1 after another, about:\n'...
               '- 1st keypress\n'...
               '--- whether the grating was tilted counterclockwise or clockwise from vertical\n' ... 
               '--- whether the grating was more or less visible than the reference (we''ll train you on this later)\n'...
               '- 2nd keypress\n'...
               '--- whether you guessed the direction of the tilt or saw it\n\n'...
               'More details on the response entry will be coming up shortly.\n\n' ...
               'When the computer has recorded your first keypress, the response cue returns to black and the crosshair at fixation ' ...
               'turns gray. You can right away make your second keypress, after which the fixation cross will turn white. This lets you know that your responses were entered successfully and the next trial is going to begin soon.\n\n'];
prompt{i}   = ['__________\nPress any key to continue.' ...
               '\nPress the left arrow key to go back.'];
waitTime(i) = shortWait;
...
%% after page 11, show keyboard practice 1

% then keyboard practice 2 

%% page 12

i = i + 1;
page{i}     = ['* Response entry\n\nOn each trial you will make two button reports, one right after another, about the patch in the response-cued quadrant:\n\n' ...
               'You will first use a single keypress to indicate BOTH the\n' ...
               '- grating tilt (counterclockwise or clockwise from vertical)\n' ...
               '- grating visibility (more or less visible than the reference you will be trained on later)\n\n' ...
               'Then, you will use a second keypress to indicate\n' ...
               '- whether you guessed the direction of the grating tilt or saw it\n\n' ...
...
%% page 13 visibility judgement 

i = i + 1;
page{i}     = ['PART 3 - doing the task\n\n* About the visibility judgment\n\n' ...
               'In addition to determining the grating''s tilt, you will report ' ...
               'whether it apears more or less visible than the reference. Let''s define these terms now.\n\n' ...
               'Visibility\n"Visibility" refers to how strongly the black and white lines appear. ' ...
               'Gratings with higher visibility appear stronger in contrast (i.e. they "pop-out" more from the noise).\n\n' ...
               'Reference\n' ...
               '- During training, the gratings indicated by the response cue will all have the same visibility. ' ...
               'This is the reference visibility.\n' ...
               '- You should note and learn this reference visibility.\n' ...
               '- In other parts of the experiment, you will be asked to judge whether post-cued gratings are ' ...
               'more or less visible than the reference visibility you learned during training.\n\n'];
prompt{i}   = ['__________\nPress any key to continue.' ...
               '\nPress the left arrow key to go back.'];
waitTime(i) = shortWait;


%% page 13+1 guessed saw tilt judgement 

i = i + 1;
page{i}     = ['PART 3 - doing the task\n\n* About the guess vs saw tilt judgment\n\n' ...
               'After the first keypress, in which you report the grating''s tilt and it''s visibility compared to a reference, you will report ' ...
               'using a second keypress whether you guessed the direction of the tilt or saw it.\n\n'...
               'There may be times when the visibility (i.e. "pop-out") of the grating ' ...
               'appears strong, and yet you couldn''t tell the direction of its tilt. And there may be other times when you see the direction of the tilt ' ...
               'even though the grating only "pops-out" weakly from the noise. So try to think of these judgments separately.\n'];
prompt{i}   = ['__________\nPress any key to continue.' ...
               '\nPress the left arrow key to go back.'];
waitTime(i) = shortWait;
...
i = i + 1;
page{i}     = ['PART 3 - doing the task\n\n* About the "clockwise" / "counterclockwise" judgments\n\n' ...
               'Sometimes you may not be very sure whether the grating was clockwise or counterclockwise. ' ...
               'If you''re not sure about the grating''s tilt, just make your best gut instinct and guess without ' ...
               ...
               'last, or always responding "clockwise"). Try to keep your "clockwise" and "counterclockwise" guesses ' ...
...
i = i + 1;
page{i}     = ['* Time limit\n\n' ...
               'After the response cue appears, you will have ' num2str(p.timing.postcueDur_inSecs) ' seconds to enter your first response. ' ...
               'If you don''t enter your responses in time, the trial will end and the ' ...
               'next one will begin. Such trials cannot be used for data analysis, so please do ' ...
               'your very best to enter responses within the time limit on every trial!\n\n' ...
               'The time limit is another reason why, if you have to make a guess, you should not spend ' ...
               'too much time deliberating, but just make a relatively quick guess based on your gut instinct, ' ...
               'or a quick "coin flip" response if you have no idea at all how to respond.\n\n'...
               'After the first keypress, you can make your second keypress right away. You will also have ' num2str(p.timing.postcue2Dur_inSecs) ' seconds to enter your second response.' ...
               'Try your best to make the two keypresses one after another, and to not spend too much time deliberating.\n\n'];
prompt{i}   = ['__________\nPress any key to continue.' ...
               '\nPress the left arrow key to go back.'];
waitTime(i) = shortWait;
...
page{i}     = ['* Stimulus frequencies and dependencies\n\n' ...
               'The gratings in each quadrant are equally likely to be clockwise or counterclockwise.' ...
               '\n\nIt is very important to note that each grating orientation in each quadrant is completely ' ...
               'independent of grating orientation in the other quadrants. In other words, knowing what was' ...
               'shown in one quadrant gives you no information whatsoever about what was shown in any other ' ...
               'quadrant.\n\n' ...
    ...
    switch KbName_oneKeyOnly(key)
        ...
        otherwise
            ...
            if j == 5
                ...
                % grating examples
                oriText = {'vertical grating', 'counterclockwise from vertical grating', 'clockwise from vertical grating'};
                i = 1;
                while i >= 1 && i <= 3
            ...
            if j == 6
                ...
                oriText = {'counterclockwise from vertical grating', 'clockwise from vertical grating'};
                i = 1;
                while i >= 1 && i <= 3
...
% 
            ...
            if j == 11 % 10
                ...
                i = i + 1;
                caption = ['stage ' num2str(i) '\nresponse cue turns white until first response entry\nhere, you''d have to respond about the grating that was presented on the upper right'];
                [textTexture, textRect] = makeTextTexture(w, caption, .25, p.stim.BGcolor);
                textRect = CenterRectOnPoint( textRect, p.rects.midW, p.rects.midH/3 );
                ...
                i = i + 1;
                caption = ['stage ' num2str(i) '\nwhen the computer successfully records your first response,\nthe response cue returns to black and the crosshair turns gray\nnow you can make your second response'];
                [textTexture, textRect] = makeTextTexture(w, caption, .25, p.stim.BGcolor);
                textRect = CenterRectOnPoint( textRect, p.rects.midW, p.rects.midH/3 );
                ...
                %% fixation cross stays gray indicating ok to make second response 
                ...
                %% fixation cross turns white after second response entry
                
                i = i + 1;
                caption = ['stage ' num2str(i) '\nwhen the computer records your second response,\nthe crosshair turns from gray to white'];
                [textTexture, textRect] = makeTextTexture(w, caption, .25, p.stim.BGcolor);
                textRect = CenterRectOnPoint( textRect, p.rects.midW, p.rects.midH/3 );
            ...
            %% after page 12 text, give response 1 entry practice
            
            if j == 12 % 11
                                
                [textTexture, textRect] = makeTextTexture(w, ['press 1, 2, 9, or 0 to see corresponding response\npress space to continue'], .3, p.stim.BGcolor);
                ...
                while ~done
                    ...
                    % frame selected oval
                    if ~isempty(pressedKey) % && pressedKey <= 4
            ...
            %% then give response 2 practice 
                                            
                [textTexture, textRect] = makeTextTexture(w, ['press 3 or 8 to see corresponding response\npress space to continue'], .3, p.stim.BGcolor);
                textRect = CenterRectOnPoint( textRect, p.rects.midW, p.rects.midH/4 );
                ...
                while ~done
                    ...
                    % draw ovals
                    for i = 1:2
                        Screen('DrawTexture', w, ovalTex2(i), [], ovalRect2{i});
                        Screen('DrawTexture', w, textTextureOval2(i), [], textRectOval2{i});                        
                    end
                    
                    % frame selected oval
                    if ~isempty(pressedKey) && pressedKey <= 2
                        Screen('FrameRect', w, [255, 0, 0], ovalRect2{pressedKey}, 10);
                    end
                    ...
                    pressedKey = find(strcmp(p.kb.respKeys2, KbName_oneKeyOnly(key)));
                    if strcmp(KbName_oneKeyOnly(key), 'space'), done = 1; end
                    if strcmp(KbName_oneKeyOnly(key), 'LeftArrow')
                        done = 1;
                        j = j - 1;
