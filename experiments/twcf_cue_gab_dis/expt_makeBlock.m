...
elseif setup.isThresholding || setup.isTraining
    ...
    % 24 trials x 5 reps = 120 trials total (40 per QUEST track, giving 40
    ...
    for i_qID = 1:3
        for i_postcueLoc = 1:4
             for i_rep = 1:nReps
                 stimID_postcue = [stimID_postcue, 1 2]; % can change to be all present for discrimination 
    ...
    %%% randomly assign contrastIDs to each uncued quadrant, while fixing
    %%% the cued quadrant contrastID = 4 
    for i_trial = 1:nTrials
        ind = randperm( 7, 4 ); % randomly choose 4 of the 7 possible line lengths
        b.contrastID_all(:, i_trial) = ind';
    
        b.contrastID_all(b.postcueLoc(i_trial), i_trial) = 4;    
        b.contrastID_postcue(i_trial)                    = 4;
    end
...
elseif setup.isValidation
    ...
    % exactly counterbalance the following:
    % (i.e. control marginal and conditional probabilities)
    % - contrast at cued location (7)
    % - stim CCW or CW for stim present trials at cue (2)
    % - postcue location (4)
    %
    % pseudo-counterbalance the following:
    % (i.e. control marginal but not conditional probabilities)
    % - grating CCW or CW for stim present trials at uncued locations
    % - BG angle
    ...
    %%% exact counterbalancing for contrast at cued loc, stimID at cued loc, and postcue loc
    stimID_postcue_list = [1 2];  % 50% stim present CCW, 50% stim present CW
    for i_contrastID = 1:7
        for i_stimID = 1:length(stimID_postcue_list)
            for i_postcueLoc = 1:4
                for i_rep = 1:nReps
                    contrastID_postcue   = [contrastID_postcue,   i_contrastID];
...
elseif setup.isPiloting
    ...
    for i_cued = 1:4
        for i_uncued = setdiff(1:4, i_cued)
            % for each possible value of cued location,
            % create psuedo even distributions of stimID in the remaining uncued locations,
            % then shuffle across trials independently for each location
            stimID_uncued{i_cued}{i_uncued}       = repmat([1 2], 1, round(nTrials/(2*4))); 
...
%% make blocks for the main experiment
    ...
    % - stim CCW/CW for stim present trials at cue (2)
    % - postcue location (4)
    %
    % pseudo-counterbalance the following:
    % (i.e. control marginal but not conditional probabilities)
    ...
    % 280 trials x 2 reps = 560 trials total, 80 trials per contrast 
    nMCU    = 280;
    nReps   = 2;
    nTrials = nMCU * nReps;
