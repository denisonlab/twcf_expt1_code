function exitNow = expt_instructions_reminder(w, p)

%% day 2 changes
page_start = 8; % attention cue
page_end = 13;
...
fontSizeMult = 0.8;
% vertical [1 2]
[textTextureOval(1), textRectOval{1}] = makeTextTexture(w, ['1 key\n\nvertical\nmore visible than reference'], 0, p.stim.BGcolor,ceil(p.text.fontSize*fontSizeMult));
[textTextureOval(2), textRectOval{2}] = makeTextTexture(w, ['2 key\n\nvertical\nless visible than reference'], 0, p.stim.BGcolor,ceil(p.text.fontSize*fontSizeMult));
% horizontal [9 0]
[textTextureOval(3), textRectOval{3}] = makeTextTexture(w, ['9 key\n\nhorizontal\nless visible than reference'], 0, p.stim.BGcolor,ceil(p.text.fontSize*fontSizeMult));
[textTextureOval(4), textRectOval{4}] = makeTextTexture(w, ['0 key\n\nhorizontal\nmore visible than reference'], 0, p.stim.BGcolor,ceil(p.text.fontSize*fontSizeMult));
...
ind_oval   = [2 3 4 5];
for i = 1:4
    [oval, ~, alpha] = imread(['supporting_files/oval' num2str(ind_oval(i))], 'png');
    oval(:, :, 4)    = alpha;
    ovalTex(i)       = Screen('MakeTexture', w, oval);
    ovalRect{i}      = CenterRectOnPoint( ceil(p.rects.periphFG{ori(i)}(:,1)'*.75), centers(ind_center(i)), p.rects.midH);

    offset = RectHeight(ovalRect{1})/2 + .3*RectHeight(ovalRect{1});
    textRectOval{i} = CenterRectOnPoint( textRectOval{i}, centers(ind_center(i)), p.rects.midH);
...
page{i}     = ['INSTRUCTIONS\n\n* Overview\n\n' ...
               ...
               '(1) What was the shape of the figure shown at this location?\n' ...
               '(2) Was it more or less visible than a reference?\n\n' ...
               'These questions will be explained in more detail as we go on.\n\n' ...
...
i = i + 1;
page{i}     = ['* Oval figures\n\nThe oriented textures contain an oval-shaped region where the lines ' ...
...
page{i}     = ['* Quadrants\n\nIn the main experiment, the screen will be split into 4 sections, called quadrants. ' ...
               ...
               'of the screen. Each quadrant contains a figure. The figure may be a horizontal oval or a vertical oval.\n\n'];
...
page{i}     = ['* Response cue (shown after the 4 quadrants)\n\nAfter the textures in the 4 quadrants are shown, one of ' ...
               ...
               'For the response-cued texture, you will enter your response about what shape the oval was (vertical or horizontal), ' ...
               'and whether it was more or less visible than a reference. More detail on response entry will be coming up shortly.\n\n' ...
...
i = i + 1;
page{i}     = ['* Response entry\n\nOn each trial you have to decide two things about the texture in the response-cued quadrant:\n' ...
               '(1) Was the figure a vertical oval (tall and narrow) or a horizontal oval (short and wide)?\n' ...
               '(2) Was it more or less visible than the reference?\n\n' ...
               'The reference level of visibility is something you learn about in an initial training stage. We''ll talk more about that shortly.\n\n' ...
...
page{i}     = ['PART 3 - doing the task\n\n* About the visibility judgment\n\n' ...
               'In addition to determining the oval''s shape (vertical or horizontal), you will report ' ...
               'whether it apears more or less visible than the reference. Let''s define these terms now.\n\n' ...
               'Visibility\n"Visibility" refers to how strongly the figure seems to "pop out" from the background texture. ' ...
               'Figures with higher visibility seem to "pop out" from the background more strongly.\n\n' ...
               'Reference\n' ...
               '- During training, the ovals indicated by the response cue will all have about the same visibility. ' ...
               'This is the reference visibility.\n' ...
               '- You should note and learn this reference visibility.\n' ...
               '- In other parts of the experiment, you will be asked to judge whether post-cued figures are ' ...
...
page{i}     = ['* Stimulus frequencies and dependencies\n\n' ...
               'The ovals in each quadrant are equally likely to be vertical or horizontal.\n\nIt is very important to note ' ...
               'that oval orientation in each quadrant is completely independent of oval ' ...
               'orientation in the other quadrants. In other words, knowing what was shown in ' ...
...
% append final page after end page
page{page_end+1}     = page{end};
prompt{page_end+1}   = prompt{end};
waitTime(page_end+1) = waitTime(end);
...
%     promptText = [prompt{j} '\n\npage ' num2str(j) ' of ' num2str(length(page))];
    promptText = [prompt{j} '\n\npage ' num2str(j-page_start+1) ' of ' num2str(page_end-page_start+2)];
