...
% b.lineLengthID_all     - 4 x N - line length at each quadrant (value=index of p.stim.periph.lineLength_inDeg_list)
...
% b.lineLengthID_postcue - 1 x N - line length ID of stimulus indicated by the postcue
...
if setup.takeScreenshot
    ...
    b.stimID_all = [1; 2; 1; 2];
    
    % lineLengthID at each quadrant. define the corresponding line length
    % in degrees in expt_param around line 108, in the if statement that reads
    % 
    % if setup.isMainExpt
    %    [...]
    % elseif setup.takeScreenshot
    %     p.stim.periph.lineLength_inDeg_list = [ENTER DESIRED VALUES HERE];
    % end
    b.lineLengthID_all = [1; 3; 5; 7];
    ...
    % determine stimID and lineLengthID at postcue
    b.stimID_postcue       = b.stimID_all( b.postcueLoc );
    b.lineLengthID_postcue = b.lineLengthID_all( b.postcueLoc );
...
elseif setup.isPractice
    ...
    stimID       = [ones(1,4), 2*ones(1,4)]';             % minimal counterbalanced unit of stimIDs
    ...
    % randomly select line length ID in each quadrant
    lineLengthID_all   = [1:7, 1:7, 1:7, 1:7]';
    lineLengthID_all   = Shuffle( repmat(lineLengthID_all, 1, nTrials) ); % make nTrials copies of lineLengthID in the columns and shuffle each column
    b.lineLengthID_all = lineLengthID_all(1:4, :);                        % select only the first 4 rows
    ...
    for i_trial = 1:nTrials
        ...
        b.lineLengthID_postcue(i_trial) = b.lineLengthID_all( b.postcueLoc(i_trial), i_trial );
    end
...
%% make training / thresholding block

elseif setup.isThresholding || setup.isTraining
    ...
    % 4 x 2 x 3 = 24 trials in minimal counterbalanced unit (MCU)
    % 24 trials x 5 reps = 120 trials total (40 per QUEST track)
    nMCU    = 24;
    nReps   = 5;
    nTrials = nMCU * nReps;
    ...
    for i_qID = 1:3
        for i_postcueLoc = 1:4
             for i_rep = 1:nReps
                 stimID_postcue = [stimID_postcue, 1 2];
                 postcueLoc     = [postcueLoc,     i_postcueLoc * ones(1,2)];
                 qID            = [qID,            i_qID * ones(1,2)];
             end
        end
    end
    ...
    for i_cued = 1:4
        for i_uncued = setdiff(1:4, i_cued)
            ...
            stimID_uncued{i_cued}{i_uncued} = repmat([1 2], 1, nTrials/(2*4));            
    ...
    %%% randomly assign lineLengthIDs to each uncued quadrant, while fixing
    %%% the cued quadrant lineLengthID = 4
    for i_trial = 1:nTrials
        ind = randperm( 7, 4 ); % randomly choose 4 of the 7 possible line lengths
        b.lineLengthID_all(:, i_trial) = ind';
    
        b.lineLengthID_all(b.postcueLoc(i_trial), i_trial) = 4;    
        b.lineLengthID_postcue(i_trial)                    = 4;
    end
    
    %%% trial and block counts
    b.nTrials         = nTrials;
    b.nTrialsPerBlock = nTrials; % 120 QUEST trials fits nicely in 1 block
    b.nBlocks         = b.nTrials / b.nTrialsPerBlock;
...
elseif setup.isValidation
    ...
    % pseudo-counterbalance the following:
    % (i.e. control marginal but not conditional probabilities)
    % - oval horizontal/vertical for stim present trials at uncued locations
    % - BG angle
    
    % 7 x 2 x 4 = 56 trials in minimal counterbalanced unit (MCU)
    % 56 trials x 2 reps = 112 trials total, 16 trials per line length
    nMCU    = 56;
    nReps   = 2;
    nTrials = nMCU * nReps;
    ...
    cueValidity          = [];
    stimID_postcue       = []; 
    lineLengthID_postcue = [];

    %%% exact counterbalancing for line length at cued loc, stimID at cued loc, and postcue loc
    stimID_postcue_list = [1 2];  % 50% stim present horizontal, 50% stim present vertical
    for i_lineLengthID = 1:7
        for i_stimID = 1:length(stimID_postcue_list)
            for i_postcueLoc = 1:4
                for i_rep = 1:nReps
                    lineLengthID_postcue = [lineLengthID_postcue, i_lineLengthID];
    ...
    % apply identical shuffling
    [b.stimID_postcue, shuffleInd] = Shuffle(stimID_postcue);
    b.lineLengthID_postcue         = lineLengthID_postcue(shuffleInd);
    b.postcueLoc                   = postcueLoc(shuffleInd);
    ...
    %%% pseudo-counterbalance lineLengthID and stimID at uncued locations
    for i_cued = 1:4
        for i_uncued = setdiff(1:4, i_cued)
            % for each possible value of cued location, create even distributions 
            % of lineLengthID and stimID in the remaining uncued locations.
            % each of the 4 cued locations has nTrials/4 trials total, so
            % for a variable with N values we need nTrials/(4*N) tilings.
            lineLengthID_uncued{i_cued}{i_uncued} = repmat(1:7, 1, nTrials/(4*7));
            stimID_uncued{i_cued}{i_uncued}       = repmat([1 2], 1, nTrials/(4*2));

            % shuffle across trials independently for each location
            lineLengthID_uncued{i_cued}{i_uncued} = Shuffle(lineLengthID_uncued{i_cued}{i_uncued});
            stimID_uncued{i_cued}{i_uncued}       = Shuffle(stimID_uncued{i_cued}{i_uncued});
        end
    end
    
    
    % combine lineLengthID_postcue with lineLengthID_uncued, and stimID_postcue with stimID_uncued
    i_cued_counter     = zeros(1,4);
    b.lineLengthID_all = nan(4, nTrials);
    b.stimID_all       = nan(4, nTrials);
    for i_trial = 1:nTrials
        ...
        % assign cued location lineLengthID and stimID
        b.lineLengthID_all(i_cued, i_trial) = b.lineLengthID_postcue(i_trial);
        b.stimID_all(i_cued, i_trial)       = b.stimID_postcue(i_trial);
        ...
        i_trial_cued = i_cued_counter(i_cued); % which trial # at cued location are we on
        for j = 1:length(i_uncued)
            b.lineLengthID_all(i_uncued(j), i_trial) = lineLengthID_uncued{i_cued}{i_uncued(j)}(i_trial_cued);
            b.stimID_all(i_uncued(j), i_trial)       = stimID_uncued{i_cued}{i_uncued(j)}(i_trial_cued);
        end
    end
...
%% make blocks for the main experiment
    ...
    % 5 x 7 x 2 x 4 = 280 trials in minimal counterbalanced unit (MCU)
    % 280 trials x 2 reps = 560 trials total, 80 trials per line length
    nMCU    = 280;
    nReps   = 2;
    nTrials = nMCU * nReps;
