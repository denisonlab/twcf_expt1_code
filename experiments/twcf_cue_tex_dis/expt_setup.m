...
stage = NaN;
while ~(stage==-1 || stage==0 || stage==1 || stage==2 || stage==3 || stage==4)
    disp(' ')
    disp('What stage?')
...
if takeScreenshot
    ...
    stageText       = {'practice', 'training', 'thresholding', 'validation', 'main_expt'};
...
setup = v2struct(exptName, exptDate, exptTime, site, resumeDataset, subjectID, dataDir, threshDir, dataFilename, ...
                 ...
                 isTraining, isThresholding, isValidation, isMainExpt, skipSyncTests, skipFlipIntervalEstimation);
