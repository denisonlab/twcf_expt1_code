...
elseif setup.isThresholding
    ...
    for i_qID = 1:3
        for i_postcueLoc = 1:4
             for i_rep = 1:nReps
                 stimID_postcue = [stimID_postcue, 0 0 1 2];
    ...
    %%% no lineLengthIDs are defined since lineLength is adjusted dynamically by QUEST
    b.lineLengthID_all     = nan(4, nTrials);
    b.lineLengthID_postcue = nan(1, nTrials);
...
%% make blocks for the main experiment
    ...
    % 560 trials x 1 reps = 560 trials total, 80 trials per line length
    nMCU    = 560;
    nReps   = 1;
    nTrials = nMCU * nReps;
